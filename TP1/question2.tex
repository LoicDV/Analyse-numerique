...
\IF {\((fa < 0 ~ et ~ fb > 0) ~ ou ~ (fa < 0 ~ et ~ fb = 0 ~ et ~ dfb > 0) \)}
	\STATE \( x \leftarrow Root1D.brent(f, a, b) \)
	\STATE ajouter x à la liste
\ELSIF {\((fa > 0 ~ et ~ fb < 0) ~ ou ~ (fa = 0 ~ et ~ fb < 0 ~ et ~ dfa < 0) \)}
	\STATE \( x \leftarrow Root1D.brent(f, a, b) \)
	\STATE ajouter x à la liste
\ELSIF {\( fa = 0 ~ et ~ fb = 0 \)}
